...
object GuiPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val typescriptSourceDirectory = settingKey[File]("Location of TypeScript sources")
    val typescriptSourceManifest  = settingKey[Seq[File]]("Ordered set of TypeScript sources that will be compiled")
  ...
  override def projectSettings = Seq(
    ...
    typescriptSourceDirectory := (sourceDirectory in Compile).value / "typescript",
    ...
    typescriptSourceManifest := Seq(
      "http.ts",
      "main.ts"
    ).map(typescriptSourceDirectory.value / _),

    update := update.dependsOn(npmInstall).value,
    resourceGenerators in Compile ++= Seq(
      // generate js from typescript sources
      Def.task {
        val log = streams.value.log
        log.info(s"Compiling TypeScript sources to JavaScript")
        val target = jsTargetDirectory.value / "main.js"
        val bin = nodeModulesBinDirectory.value
        Process(s"$bin/tsc --outFile $target " + typescriptSourceManifest.value.mkString(" ")) ! log
        Seq(target)
      }.taskValue,
